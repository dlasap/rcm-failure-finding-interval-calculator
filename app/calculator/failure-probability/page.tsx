...
import { calculateFailureProbability, calculateFailureRateFromMTBF, convertMTBF } from '@/lib/formulaProcessor'
...
export default function FailureProbabilityCalculator() {
  const [result, setResult] = useState<FailureProbabilityInputs & { probability: number } | null>(null);
  const [chartData, setChartData] = useState<any[]>([]);

  const { register, handleSubmit, formState: { errors } } = useForm<FailureProbabilityInputs>({
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Enter the failure rate, inspection interval, and costs to calculate the probability of failure and cumulative costs over time.</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      {result && (
        <Card>
          ...
          <CardContent>
            <p className="mb-4">Probability of failure: {formatNumber(result.probability.toFixed(4))}</p>
                ...
                <LineChart data={chartData}>
                  ...
                  <Tooltip 
                    formatter={(value, name, props) => [
                      name === 'probability' ? formatNumber(Number(value).toFixed(4)) : `${formatNumber(Number(value).toFixed(2))} EUR`, 
                      name === 'probability' ? 'Probability of Failure' : 'Cumulative Cost'
                    ]}
                    labelFormatter={(label) => {
                      if (result.inspectionInterval >= 8760) return `${formatNumber((label / 8760).toFixed(1))} Years`;
                      if (result.inspectionInterval >= 730) return `${formatNumber((label / 730).toFixed(1))} Months`;
                      return `${formatNumber((label / 168).toFixed(1))} Weeks`;
                    }}
                  />
                  <Legend />
            ...
            <p className="mb-4">
              The graph shows both the probability of failure and the cumulative cost over time. 
              The probability of failure increases over time, while the cumulative cost includes both inspection costs and expected failure costs. 
              The x-axis is scaled to show time in {result.inspectionInterval >= 8760 ? 'years' : result.inspectionInterval >= 730 ? 'months' : 'weeks'}, 
              providing a clear view of how both probability and costs evolve over a relevant timeframe.
            </p>
          </CardContent>
        </Card>
      )}
