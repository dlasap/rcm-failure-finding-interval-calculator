    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Enter either the failure rate or MTBF, along with the inspection interval and target reliability to calculate and compare the reliability.
          </CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <p className="mb-4">
              ...
              {result.reliability < 0.01
                ? `${formatNumber(Number((result.reliability * 100).toFixed(2)))}%`
                : `${formatNumber(Number(result.reliability.toFixed(4)))} (${formatNumber(Number((result.reliability * 100).toFixed(2)))}%)`}
            </p>
            <p className="mb-4">
              Status:{" "}
              {result.meetingTarget ? <span className="text-green-500">Meeting target</span> : <span className="text-red-500">Below target</span>}
            </p>
            <h3 className="text-lg font-semibold mb-2">Explanation:</h3>
            <p className="mb-4">
              Reliability is the probability that the system or component will perform its intended function for a specified period of time under
              stated conditions. Higher reliability indicates better performance and lower chance of failure.
            </p>
