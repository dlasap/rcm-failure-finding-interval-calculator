      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Button type="submit" className="w-full bg-secondary text-secondary-foreground hover:bg-secondary/90">Calculate FFI</Button>
          </form>
        </CardContent>
      </Card>
      {result !== null && (
        <Card>
          ...
          <CardContent>
            <p className="mb-4 text-lg">Optimal Failure Finding Interval: <span className="font-semibold">{formatNumber(result.toFixed(2))} hours</span></p>
            <h3 className="text-xl font-semibold mb-2 text-primary">Explanation:</h3>
            <p className="mb-4">
              The Failure Finding Interval (FFI) is the optimal time between inspections for the voting system. 
              This interval balances the need for frequent checks against the cost and disruption of inspections. 
              The calculated FFI of {result.toFixed(2)} hours suggests that the voting system should be inspected 
              approximately every {(result / 24).toFixed(2)} days to maintain optimal reliability and detect potential failures.
            </p>
            ...
            <p className="mb-4">
              The graph shows how the system reliability decreases over time (blue line) and how the probability of 
              detecting a failure increases (orange line). The optimal FFI strikes a balance between these two factors, 
              ensuring that inspections are frequent enough to catch failures before they significantly impact the 
              voting system's reliability, but not so frequent as to be unnecessarily disruptive or costly.
            </p>
          </CardContent>
        </Card>
      )}
