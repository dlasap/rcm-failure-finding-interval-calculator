...
export default function OptimalIntervalCalculator() {
  ...
  const onOptimalIntervalSubmit: SubmitHandler<OptimalIntervalInputs> = (data) => {
    ...
    for (let i = 0; i <= maxInterval; i += maxInterval / dataPoints) {
      const inspectionCost = isFinite(data.inspectionCost * (i / optimalIntervalHours)) ? 
        data.inspectionCost * (i / optimalIntervalHours) : 0;
      const failureCost = isFinite(data.failureCost * data.failureRate * i / 2) ? 
        data.failureCost * data.failureRate * i / 2 : 0;
    ...
    <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={mtbfForm.handleSubmit(onMTBFSubmit)} className="space-y-4">
              ...
              <Button type="submit" className="w-full bg-secondary text-secondary-foreground hover:bg-secondary/90">Calculate Failure Rate</Button>
            </form>
          </CardContent>
        </Card>
      )}
          ...
          <CardContent>
            <form onSubmit={optimalIntervalForm.handleSubmit(onOptimalIntervalSubmit)} className="space-y-4">
              <div>
                ...
                <Input id="failureRate" type="number" step="0.0001" {...optimalIntervalForm.register('failureRate', { valueAsNumber: true })} readOnly />
              </div>
              <div>
                ...
                {optimalIntervalForm.formState.errors.inspectionCost && <span className="text-destructive">This field is required and must be non-negative</span>}
              </div>
              <div>
                ...
                {optimalIntervalForm.formState.errors.failureCost && <span className="text-destructive">This field is required and must be positive</span>}
              </div>
              <Button type="submit" className="w-full bg-secondary text-secondary-foreground hover:bg-secondary/90">Calculate Optimal Interval</Button>
            </form>
          </CardContent>
          <CardFooter>
            <Button variant="outline" onClick={() => setStep(1)} className="w-full">Back to Step 1</Button>
          </CardFooter>
        </Card>
      )}
      ...
      {result !== null && (
        <Card>
          ...
          <CardContent>
            <p className="mb-4 text-lg">Optimal Inspection Interval: <span className="font-semibold">{formatNumber(result.value.toFixed(2))} {result.unit}</span></p>
            <p className="mb-4 text-lg">Used Failure Rate: <span className="font-semibold">{formatNumber(calculatedFailureRate?.toFixed(6))} failures/hour</span></p>
            <h3 className="text-xl font-semibold mb-2 text-primary">Explanation:</h3>
            <p className="mb-4">
              The optimal inspection interval has been calculated and converted to a more readable format. 
              This interval of {result.value.toFixed(2)} {result.unit} represents the most cost-effective time 
              between inspections, balancing the cost of inspections against the potential cost of failures. 
              The graph below shows the cumulative cost over time, including both inspection costs and expected failure costs, 
              as well as the probability of unidentified failure.
            </p>
            ...
            <p className="mb-4">
              Adjust your inspection interval to match the calculated optimal interval as closely as possible. 
              However, also consider practical constraints such as resource availability and regulatory requirements. 
              Regularly reassess this interval as costs or failure rates change.
            </p>
              ...
              >
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                    ...
                    <Line yAxisId="right" type="monotone" dataKey="probabilityOfUnidentifiedFailure" stroke="var(--color-probabilityOfUnidentifiedFailure)" name="Probability of Unidentified Failure" />
                  </LineChart>
                </ResponsiveContainer>
              </ChartContainer>
            </div>
            ...
            <p className="mb-4">
              The graph shows how costs and the probability of unidentified failure change over time. The optimal interval 
              is where the cumulative cost is at its minimum. The probability of unidentified failure increases over time, 
              which is why regular inspections are crucial. Balance this probability against the costs to determine the best 
              inspection strategy for your specific situation.
            </p>
            <p className="mb-4">
